...
public abstract class BaseBehavior extends AppBarLayout.Behavior implements OnSmoothScrollListener {
     ...
     * @return
     */
    final boolean fling(AppBarLayout layout, View target, float velocityY) {
        ...
        if (canScroll) {
            mFlingRunnable = new FlingRunnable(layout, target, velocityY);
    ...
    private class FlingRunnable implements Runnable {
        ...
        FlingRunnable(AppBarLayout layout, View target, float velocityY) {
            mLayout = layout;
            scrollTarget = target;
            this.mLastY = 0;
    ...
    public boolean onStartNestedScroll(CoordinatorLayout parent, AppBarLayout child, View directTargetChild, View target, int nestedScrollAxes, int type) {
        Logd(TAG, "NestedScrollingParent, onStartNestedScroll = [" + nestedScrollAxes + "]");
...
//            int flingDuration = getSplineFlingDuration((int) velocityY);
            ...
            Logd(TAG, "NestedScrollingParent: fling:  flingY:" + flingY + ",subV:" + subV);
            fling(child, mScrollTarget, -Math.abs(subVelocity2));
        }
    }
    ...
    public void onStopNestedScroll(CoordinatorLayout coordinatorLayout, AppBarLayout abl, View target, int type) {
        Loge(TAG, "NestedScrollingParent: onStopNestedScroll() fling:  type0:" + type);
    ...
    protected void syncOffset(int newOffset, final int mTotalScrollY) {

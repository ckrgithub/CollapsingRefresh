...
import com.ckr.smartrefresh.api.DefaultRefreshFooterCreater;
import com.ckr.smartrefresh.api.DefaultRefreshHeaderCreater;
import com.ckr.smartrefresh.api.RefreshContent;
...
import com.ckr.smartrefresh.api.RefreshScrollBoundary;
import com.ckr.smartrefresh.constant.DimensionStatus;
import com.ckr.smartrefresh.constant.RefreshState;
import com.ckr.smartrefresh.constant.SpinnerStyle;
import com.ckr.smartrefresh.footer.ClassicsFooter;
import com.ckr.smartrefresh.header.ClassicsHeader;
import com.ckr.smartrefresh.impl.RefreshContentWrapper;
import com.ckr.smartrefresh.impl.RefreshFooterWrapper;
...
import com.ckr.smartrefresh.util.PostRunable;
import com.ckr.smartrefresh.util.ViscousFluidInterpolator;
...
import static com.ckr.smartrefresh.util.DensityUtil.dp2px;
...
public class SmartRefreshLayout extends ViewGroup implements RefreshLayout {
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, com.ckr.smartrefresh.R.styleable.SmartRefreshLayout);

        ViewCompat.setNestedScrollingEnabled(this, ta.getBoolean(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlEnableNestedScrolling, false));
        mDragRate = ta.getFloat(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlDragRate, mDragRate);
        mHeaderMaxDragRate = ta.getFloat(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlHeaderMaxDragRate, mHeaderMaxDragRate);
        mFooterMaxDragRate = ta.getFloat(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlFooterMaxDragRate, mFooterMaxDragRate);
        mEnableRefresh = ta.getBoolean(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlEnableRefresh, mEnableRefresh);
        mReboundDuration = ta.getInt(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlReboundDuration, mReboundDuration);
        mEnableLoadmore = ta.getBoolean(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlEnableLoadmore, mEnableLoadmore);
        mHeaderHeight = ta.getDimensionPixelOffset(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlHeaderHeight, density.dip2px(100));
        mFooterHeight = ta.getDimensionPixelOffset(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlFooterHeight, density.dip2px(60));
        mDisableContentWhenRefresh = ta.getBoolean(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlDisableContentWhenRefresh, mDisableContentWhenRefresh);
        mDisableContentWhenLoading = ta.getBoolean(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlDisableContentWhenLoading, mDisableContentWhenLoading);
        mEnableHeaderTranslationContent = ta.getBoolean(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlEnableHeaderTranslationContent, mEnableHeaderTranslationContent);
        mEnableFooterTranslationContent = ta.getBoolean(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlEnableFooterTranslationContent, mEnableFooterTranslationContent);
        mEnablePreviewInEditMode = ta.getBoolean(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlEnablePreviewInEditMode, mEnablePreviewInEditMode);
        mEnableAutoLoadmore = ta.getBoolean(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlEnableAutoLoadmore, mEnableAutoLoadmore);
        mEnableOverScrollBounce = ta.getBoolean(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlEnableAutoLoadmore, mEnableOverScrollBounce);
        mEnablePureScrollMode = ta.getBoolean(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlEnablePureScrollMode, mEnablePureScrollMode);
        mEnableScrollContentWhenLoaded = ta.getBoolean(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlEnableScrollContentWhenLoaded, mEnableScrollContentWhenLoaded);
        mFixedHeaderViewId = ta.getResourceId(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlFixedHeaderViewId, View.NO_ID);
        mFixedFooterViewId = ta.getResourceId(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlFixedFooterViewId, View.NO_ID);

        mManualLoadmore = ta.hasValue(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlEnableLoadmore);
        mManualNestedScrolling = ta.hasValue(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlEnableNestedScrolling);
        mHeaderHeightStatus = ta.hasValue(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlHeaderHeight) ? DimensionStatus.XmlLayoutUnNotify : mHeaderHeightStatus;
        mFooterHeightStatus = ta.hasValue(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlFooterHeight) ? DimensionStatus.XmlLayoutUnNotify : mFooterHeightStatus;
        ...
        int accentColor = ta.getColor(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlAccentColor, 0);
        int primaryColor = ta.getColor(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_srlPrimaryColor, 0);
        if (primaryColor != 0) {
            if (accentColor != 0) {
    ...
    protected void onFinishInflate() {
        ...
        Log.w(TAG, "onFinishInflate: count:" + count);
        //定义为确认的子View索引
        boolean[] uncertains = new boolean[count];
        //第一次查找确认的 子View
        for (int i = 0; i < count; i++) {
            View view = getChildAt(i);
            Log.w(TAG, "onFinishInflate: view:" + view);
        ...
        for (int i = 0; i < count; i++) {
            if (uncertains[i]) {
                View view = getChildAt(i);
                Log.d(TAG, "onFinishInflate: uncertains:" + view);
    ...
    protected void onAttachedToWindow() {
        ...
        Log.d(TAG, "onFinish,onAttachedToWindow: ");
                ...
                if (!mIsBeingDragged) {
                    if (Math.abs(dy) >= mTouchSlop && Math.abs(dx) < Math.abs(dy)) {//滑动允许最大角度为45度
                        ...
                        if (dy > 0 && (mSpinner < 0 || (mEnableRefresh && mRefreshContent.canRefresh()))) {
                            ...
                            Log.d(TAG, "ScrollBoundaryUtil  ");
                            Log.d(TAG, "dispatchTouchEvent222: mSpinner:" + mSpinner + ",dy:" + dy);
                            if (mSpinner > 0) {
                                setStatePullDownToRefresh();
                            } else {
                                setStatePullUpToLoad();
...
//                        return super.dispatchTouchEvent(e);
    ...
    protected void setStateRefresing() {
        ...
        Log.d(TAG, "setStateRefresing: mLastRefreshingTime:" + mLastRefreshingTime);
    ...
    protected ValueAnimator animSpinner(int endSpinner, int startDelay, Interpolator interpolator) {
        Log.d(TAG, "animSpinner: 回弹动画：mSpinner:" + mSpinner + ",endSpinner:" + endSpinner);
    ...
    protected boolean overSpinner() {
        if (mState == RefreshState.Loading) {
            ...
            Log.d(TAG, "overSpinner: mSpinner:" + mSpinner + ",mHeaderHeight:" + mHeaderHeight);
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        public LayoutParams(Context context, AttributeSet attrs) {
            super(context, attrs);
            TypedArray ta = context.obtainStyledAttributes(attrs, com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_Layout);
            backgroundColor = ta.getColor(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_Layout_srlBackgroundColor, backgroundColor);
            if (ta.hasValue(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_Layout_srlSpinnerStyle)) {
                spinnerStyle = SpinnerStyle.values()[ta.getInt(com.ckr.smartrefresh.R.styleable.SmartRefreshLayout_Layout_srlSpinnerStyle, SpinnerStyle.Translate.ordinal())];
            }
            ta.recycle();
        }
...
//        }
    ...
    public SmartRefreshLayout finishRefresh() {
        ...
        Log.d(TAG, "finishRefresh111: passTime:" + passTime + ",mLastRefreshTime:" + mLastRefreshingTime);
    ...
    public SmartRefreshLayout finishRefresh(int delayed, final boolean success) {
        ...
        Runnable runnable = new Runnable() {
            @Override
            public void run() {
                Log.d(TAG, "run: 状态：mState:" + mState);
                if (mState == RefreshState.Refreshing) {
                    Log.d(TAG, "run: 刷新状态");
                    if (mRefreshHeader != null) {
                        Log.d(TAG, "run: 头部不为null");
                        ...
                        if (startDelay < Integer.MAX_VALUE) {
                            if (mSpinner == 0) {
                                ...
                                Log.d(TAG, "run: startDelay");
                                animSpinner(0, startDelay);
                            }
                        }
                    } else {
                        Log.d(TAG, "run: 头部为null");
                        resetStatus();
                    }
                }
            }
    ...
    public boolean autoRefresh(int delayed, final float dragrate) {
        Log.d(TAG, "autoRefresh: drag_rate:" + dragrate + ",mState:" + mState + ",mEnableRefresh:" + mEnableRefresh);
        if (mState == RefreshState.None && mEnableRefresh) {
            ...
            Log.d(TAG, "autoRefresh: delayed:" + delayed);
            if (delayed > 0) {
                postDelayed2(runnable, delayed);
            } else {
                runnable.run();
